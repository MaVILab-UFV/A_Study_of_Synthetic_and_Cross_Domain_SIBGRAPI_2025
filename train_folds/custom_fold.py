...
class CustomDataset(Dataset):
    def __init__(self, img_dir, mask_dir, colormap, crop_size=(480, 480), final_size=(128, 128), 
        ...
        self.img_files = sorted(
            ...
            key=lambda x: tuple(map(int, x.split('.')[0].split('_')[-1].split(',')))
        )
    ...
    def random_crop(self, image, mask, size):
        ...
        if height < crop_height or width < crop_width:
            raise ValueError("The image or mask is smaller than the crop size.")
    ...
    def random_flip(self, image, mask):
        flip_type = random.choice([None, 0, 1, -1])  # None: no flip, 0: vertical, 1: horizontal, -1: both
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            ...
            #print("✅ Random rotate.")     
            image, mask = self.random_mirror(image, mask)    

        # Save images after cropping, before resizing
        if self.debug_mode and (image_file, mask_file) in self.debug_samples:
            ...
            #cv.imwrite(mask_cropped_path, mask) 
            cv.imwrite(image_cropped_path, cv.cvtColor(image, cv.COLOR_RGB2BGR))
            cv.imwrite(mask_cropped_path, mask)
        ...
        if self.debug_mode and (image_file, mask_file) in self.debug_samples:
            ...
            cv.imwrite(image_resized_path,  cv.cvtColor(image_resized, cv.COLOR_RGB2BGR))
            cv.imwrite(mask_resized_path,  mask_resized)
    ...
    def _filter_pairs(self, img_files, mask_files):
        ...
        for img, mask in zip(img_files, mask_files):
            ...
            if os.path.exists(img_path) and os.path.exists(mask_path):
                ...
                print(f"Image or segment not found for: {img} or {mask}")
