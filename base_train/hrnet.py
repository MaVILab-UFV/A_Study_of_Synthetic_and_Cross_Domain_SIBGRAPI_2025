            ...
            for j in range(1, self.num_branches):
                if i == j:
                    ...
                    y = y + F.interpolate(
                        ...
                        mode="bilinear",
                        align_corners=self.align_corners, #ALIGN_CORNERS,
                    )
                else:
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, in_channels, n_classes, timestamps, **kwargs):
        
        super(HighResolutionNet, self).__init__()
        
        extra = {
            "STAGE1": {"BLOCK": "BOTTLENECK", "NUM_BLOCKS": [4], "NUM_CHANNELS": [64]},
            "STAGE2": {
                "BLOCK": "BASIC",
                "FUSE_METHOD": "SUM",
                "NUM_BLOCKS": [4, 4],
            ...
            "OCR": {"KEY_CHANNELS": 256, "MID_CHANNELS": 512},
        }
        
        if type(in_channels) is list:
            self.in_channels = sum(in_channels)
        elif type(in_channels) is int:
            self.in_channels = in_channels
        ...
        # stem net
        self.conv1 = nn.Conv2d(self.in_channels * self.timestamps, 64, kernel_size=3, stride=1, padding=1, bias=False)
        # self.conv1 = nn.Conv2d(self.in_channels * self.timestamps, 64, kernel_size=3, stride=2, padding=1, bias=False)
        ...
        # self.upsample = nn.ConvTranspose2d(last_inp_channels, last_inp_channels, kernel_size=4, stride=2, padding=1)
